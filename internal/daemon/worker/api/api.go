...
type (
	...
	Service interface {
		ScheduleWorkflow(ctx context.Context, name string, runID string, input []byte) error
		ScheduleTask(ctx context.Context, name string, runID string, input []byte) error
	}
)
